...
当前版本：**v2.1.0** · [中英双语更新说明](CHANGELOG.md)
...
- 查看各文件系统的磁盘容量。按用户的磁盘归属（每日/手动 `find` 扫描）因占用内存和磁盘较多，在本版本中**冻结**：代码保留在功能开关之后，计划后续下架。
...
记录粒度与实时刷新相互独立：服务器卡片按所选采样间隔更新，但本地历史**每台服务器每个 UTC 分钟最多落盘一条记录**，无论采样多快都不会让历史文件膨胀。历史查询按时间戳预过滤流式解析日文件，不再整文件载入内存。（各窗口每 5 分钟的磁盘归属自动刷新已随按用户磁盘归属功能冻结而关闭。）
...
- 标题栏（窗口控制按钮）与在线统计/刷新间隔行固定在窗口顶部不滚动；服务器卡片放不下时只有卡片列表在窗口内部滚动，任意滚动位置下最小化/关闭按钮始终可见可点（v2.1.3）。

## 指标与用户明细

- **CPU**：整台服务器 CPU 百分比。
- **MEM**：百分比以及 已用/总内存（总量），例如 `73.0% · 92.2 / 125.5 GB`。
- 磁盘使用情况统一在 GPU 卡片下方的“全部磁盘”中查看，不再显示独立的 **DISK** 概览框。
...
GPU 卡片下方是磁盘区域：点击“全部磁盘”展开（或收起）所有真实文件系统（自动排除 snap/squashfs 回挂载、tmpfs/overlay 等虚拟文件系统），每个挂载点一行，显示百分比、已用/总容量和进度条。

**按用户磁盘归属已冻结。** 悬停/点击弹出的按用户磁盘归属面板、“立即扫描”按钮、每日 `find` 扫描和记录页的按用户磁盘曲线都通过代码开关（`DISK_ATTRIBUTION_FROZEN`）关闭——因为该扫描在远端服务器上非常占用内存和磁盘。功能代码保留在仓库中，计划后续整体下架而非直接删除。已有的归属历史文件原样保留、不做清理；只是不再产生新扫描和新记录。如需重新启用，需同时翻转 Rust 与前端两层的开关（见 `docs/DEVELOPMENT.md`）。
...
- 磁盘视图：每个挂载点一条使用率曲线（按用户已用容量 GB 的日级阶梯曲线随按用户磁盘归属功能一同冻结）；
- 按分钟定位，浮窗显示完整时间和同一分钟的全部指标；
- 每台服务器的 RAM 芯片与内存浮窗/固定弹窗显示系统总内存，例如 `55.1% / 48.2% · 128 GB`（旧记录无总量时自动省略）；
- 单击曲线固定浮窗，双击解除固定；
...
└─ history\\
   ...
   └─ attribution\\
      └─ yyyy-MM-dd.jsonl  # 每次扫描每个挂载点一行磁盘归属记录（UTC 日期文件；已冻结——原样保留，不再新增）
```
...
- 磁盘归属记录合并时按 `(serverId, mount, scannedAt)` 去重，重复合并不产生重复数据。记录只包含 uid、用户名、已用字节数和挂载点路径——绝不包含 PID、进程名、命令行或密码。（已冻结：功能冻结期间不再写入新的归属记录，已有文件原样保留。）
...
├─ scan.sh                # 磁盘归属扫描脚本（已冻结——不再部署或调度）
├─ config                 # 采样间隔、保留天数、扫描开关与时刻（已冻结——恒写为关闭）、服务器身份
├─ state/                 # pid/心跳文件，以及扫描器的 scan.lock / scan.status
├─ attribution/yyyy-MM-dd.jsonl  # 每次扫描每个挂载点一行归属记录（已冻结——原样保留，不再清理或拉取）
├─ records/yyyy-MM-dd.v2.jsonl  # 分钟记录，与本地历史同格式（UTC 时间戳）
└─ agent.log              # 代理输出；扫描器输出追加到 scan.log
```
...
- **配置**：设置采样间隔（1–3600 秒）、服务器端保留天数（1–3650），以及应用启动时是否自动恢复已停止的代理。（按用户磁盘归属冻结期间，每日磁盘归属扫描开关与时刻被隐藏；代理配置恒以扫描关闭写入。）
...
- 更新 Server Pulse 后，对已有的服务器端代理执行一次**重启**或**注入**，让它获得当前聚合脚本。按用户磁盘归属冻结期间，每次写入的代理配置都强制扫描为关闭——以旧版“扫描开启”配置部署的已有代理，在你执行一次**重启**、**注入**或在“配置”中保存后才会停止每日 `find` 扫描的调度。此前生成的记录无法回溯重建用户明细。
- 服务器重启会使代理停止（普通用户进程），徽标显示“已停止”；可手动重新注入，或在“配置”中开启启动时自动恢复。
- 记录内容与本地历史一致：UID、用户名、分钟聚合，绝不包含 PID、进程名、命令行或密码；磁盘归属记录额外包含挂载点路径（属运维信息）和每用户已用字节数。
- 服务器需要 POSIX `sh`、`awk`、`/proc`，可选 `nvidia-smi`——与实时监控要求相同。（已冻结的磁盘扫描还曾需要 `find`、`df`，以及可用时的 `nice`/`ionice`/`timeout`；现已不再运行。）
...
├─ history\\            # 分钟历史
│  └─ attribution\\     # 磁盘归属镜像记录（UTC 日期文件；已冻结——原样保留，不再新增）
└─ error.log           # UI/历史异常摘要
```
...
├─ assets/serverpulse-scan.sh   # canonical、仅 LF 的磁盘归属扫描脚本（已冻结——保留待后续下架）
...
**主界面显示“没有选择服务器”，但管理页的复选框已勾选？或管理页勾选/取消勾选后主界面要退出重进才生效？** 主界面和管理页是独立 WebView。应用会在加载 SSH 配置前先注册跨窗口 `servers-changed` 事件，并忽略较旧的初始服务器列表，避免启动期间保存的勾选状态被旧响应覆盖。所有 Tauri 事件名不允许包含点号（Tauri 2 只允许字母、数字与 `-`、`/`、`:`、`_`）——含点号的事件名会在 `listen` 时被拒绝、在 `emit` 时被静默丢弃，导致跨窗口同步失效。请先关闭旧的应用实例，再启动重新构建的 portable EXE。
