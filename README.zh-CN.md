...
- 查看各文件系统的磁盘容量与按用户的磁盘归属（每日自动扫描或手动触发）。
...
记录粒度与实时刷新相互独立：服务器卡片按所选采样间隔更新，但本地历史**每台服务器每个 UTC 分钟最多落盘一条记录**，无论采样多快都不会让历史文件膨胀。历史查询按时间戳预过滤流式解析日文件，不再整文件载入内存；各窗口每 5 分钟的磁盘归属自动刷新也只读取轻量的归属文件，不触碰完整指标历史。
...
悬停或点击 **DISK** 数值会弹出最近一次服务端扫描的按用户磁盘归属面板。GPU 卡片下方是磁盘区域：点击“全部磁盘”展开（或收起）所有真实文件系统（自动排除 snap/squashfs 回挂载、tmpfs/overlay 等虚拟文件系统），每个挂载点一行，显示百分比、已用/总容量和进度条；悬停或点击某行查看该挂载点的归属面板。还没有扫描数据时，面板会明确说明原因，而不是显示为零。

展开的磁盘区域内有“立即扫描”按钮：以低优先级在服务器上后台启动基于 `find` 的扫描（脱离 SSH 会话执行），关闭应用扫描不会中断。扫描进行中按钮禁用，并显示已运行时长和当前正在扫描的挂载点。检测到扫描完成后会自动拉取并合并结果到本地历史，归属面板无需手动合并即可更新。
...
- 磁盘视图：每个挂载点一条使用率曲线，外加最多 3 条按用户已用容量（GB）的日级阶梯曲线（同一次扫描跨挂载点求和）；
...
└─ history\\
   ...
   └─ attribution\\
      └─ yyyy-MM-dd.jsonl  # 每次扫描每个挂载点一行磁盘归属记录（UTC 日期文件）
```
...
- 磁盘归属记录合并时按 `(serverId, mount, scannedAt)` 去重，重复合并不产生重复数据。记录只包含 uid、用户名、已用字节数和挂载点路径——绝不包含 PID、进程名、命令行或密码。
...
├─ scan.sh                # 磁盘归属扫描脚本，随代理部署或按需现场写入
├─ config                 # 采样间隔、保留天数、扫描开关与时刻、服务器身份
├─ state/                 # pid/心跳文件，以及扫描器的 scan.lock / scan.status
├─ attribution/yyyy-MM-dd.jsonl  # 每次扫描每个挂载点一行归属记录
...
- **配置**：设置采样间隔（1–3600 秒）、服务器端保留天数（1–3650）、每日磁盘归属扫描开关与时刻（默认开启，服务器本地小时 0–23，默认 3 点时段——代理在该时刻后最多触发一次当日扫描），以及应用启动时是否自动恢复已停止的代理。
...
- 更新 Server Pulse 后，对已有的服务器端代理执行一次**重启**或**注入**，让它获得当前聚合脚本并启用每日磁盘扫描调度；手动“立即扫描”不受影响（触发时会现场部署当前扫描脚本）。此前生成的记录无法回溯重建用户明细。
...
- 服务器需要 POSIX `sh`、`awk`、`/proc`，可选 `nvidia-smi`——与实时监控要求相同。磁盘扫描还会使用 `find`、`df`，以及在可用时的 `nice`/`ionice`/`timeout`。
...
├─ assets/serverpulse-scan.sh   # canonical、仅 LF 的磁盘归属扫描脚本
