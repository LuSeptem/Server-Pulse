...
$script:historyStorageContext = [PSCustomObject]@{
    ...
    ApplyRoot={param($root,$settingsObject);$script:settingsDirectory=$root;$script:settingsPath=Join-Path $root 'settings.json';$script:serverStorePath=Join-Path $root 'servers.json';$script:historyRecorder.Directory=Join-Path $root 'history';if($null -eq $historyAskPassCommand){throw 'SSH helper is unavailable. Please restart Server Pulse.'};if($null -eq $historyContextSetterCommand){throw 'History storage context helper is unavailable. Please restart Server Pulse.'};$script:askPassPath=& $historyAskPassCommand -Directory (Join-Path $root 'bin');[void](& $historyContextSetterCommand -Context $script:historyStorageContext -Name 'ActiveRoot' -Value $root);[void](& $historyContextSetterCommand -Context $script:historyStorageContext -Name 'PreferredRoot' -Value $root);[void](& $historyContextSetterCommand -Context $script:historyStorageContext -Name 'IsFallback' -Value $false);[void](& $historyContextSetterCommand -Context $script:historyStorageContext -Name 'PendingSync' -Value $false)}.GetNewClosure()
