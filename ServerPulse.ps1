...
$historyStorageContext.SaveSettings={param($settingsObject,$root);if($null -ne $settingsObject){$script:settings=$settingsObject};if($root){$script:settingsDirectory=$root;$script:settingsPath=Join-Path $root 'settings.json'};if($null -eq $historyJsonWriterCommand){throw 'Settings writer is unavailable. Please restart Server Pulse.'};& $historyJsonWriterCommand -Path $script:settingsPath -Value $script:settings -Depth 8}.GetNewClosure()
$historyStorageContext.ApplyRoot={param($root,$settingsObject);$script:settingsDirectory=$root;$script:settingsPath=Join-Path $root 'settings.json';$script:serverStorePath=Join-Path $root 'servers.json';$script:historyRecorder.Directory=Join-Path $root 'history';if($null -eq $historyAskPassCommand){throw 'SSH helper is unavailable. Please restart Server Pulse.'};if($null -eq $historyContextSetterCommand){throw 'History storage context helper is unavailable. Please restart Server Pulse.'};$script:askPassPath=& $historyAskPassCommand -Directory (Join-Path $root 'bin');[void](& $historyContextSetterCommand -Context $historyStorageContext -Name 'ActiveRoot' -Value $root);[void](& $historyContextSetterCommand -Context $historyStorageContext -Name 'PreferredRoot' -Value $root);[void](& $historyContextSetterCommand -Context $historyStorageContext -Name 'IsFallback' -Value $false);[void](& $historyContextSetterCommand -Context $historyStorageContext -Name 'PendingSync' -Value $false)}.GetNewClosure()
$historyStorageContext.OnRequery={}.GetNewClosure()
$script:cards = @{}
