...
历史文件只保存在当前 Windows 用户本机：

```text
%LOCALAPPDATA%\ServerPulse\history\
...
- 新记录按本地日期追加到 `yyyy-MM-dd.v2.jsonl`，默认保留 30 天。
...
- 历史清理会删除超过 30 天的 `.json` 和 `.jsonl` 文件。删除整个 `history` 目录只会清空记录，不影响窗口设置或服务器配置。
...
├─ settings.json       # 主题、语言、窗口位置/尺寸、透明度、刷新间隔等
...
仓库中的 `config/servers.json` 只是首次运行的种子配置，不保存密码：
