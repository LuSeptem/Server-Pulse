...
- 原生历史窗口支持分钟精度的起止时间查询，默认显示最近 1 小时，并按服务器和 GPU 显示曲线；鼠标悬停时按横轴分钟统一定位，以一条竖线和多个圆点同时标记该分钟的全部曲线，并显示完整日期、时间及所有指标值。
...
冒烟模式会实际运行一次 SSH 采集，验证 WPF 主窗口与历史窗口、应用内坐标拖拽、历史窗口关闭按钮的命中、事件隔离与实际关闭、刷新间隔、分钟历史聚合、默认最近一小时、分框日期输入与红框/`!` 越界提示、折线图同分钟多指标悬停、完整时间显示及移出隐藏、损坏历史文件的查询异常拦截、未处理 UI 事件的应用级保护、仅背景透明、逐卡显存数据和贴边隐藏，然后生成 `tests/artifacts/native-window.png` 与 `tests/artifacts/history-window.png` 并自动退出。
