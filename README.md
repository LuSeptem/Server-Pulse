...
冒烟模式会实际运行一次 SSH 采集，验证 WPF 主窗口与历史窗口、应用内坐标拖拽、刷新间隔、分钟历史聚合、默认最近一小时、分框日期输入与红框/`!` 越界提示、损坏历史文件的查询异常拦截、仅背景透明、逐卡显存数据和贴边隐藏，然后生成 `tests/artifacts/native-window.png` 与 `tests/artifacts/history-window.png` 并自动退出。
